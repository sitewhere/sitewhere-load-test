  @echo off
  echo.
  echo   Load Test Start Script for Windows v1.0
  echo.
...
:CHECKFILE
  ...
  FOR %%i IN (%SITEWHERE_FOLDER%/lib/loadtest.war) DO IF EXIST %%~si  GOTO START
  echo ERROR! Load test WAR not found.
  GOTO EXIT
...
:START
  ...
  java %JAVA_OPTS% %JPDA_OPTS% -jar %SITEWHERE_FOLDER%/lib/loadtest.war
  GOTO EXIT
