...
public class AgentsParser extends AbstractBeanDefinitionParser {
	...
	context.getRegistry().registerBeanDefinition(ILoadTestBeans.BEAN_AGENT_MANAGER, manager.getBeanDefinition());
	return null;
    }
	...
	for (Element child : children) {
		...
		if (nested != null) {
		    ...
		    throw new RuntimeException("Invalid nested element found in 'agents' section: " + child.toString());
		}
	    }
	...
	Attr serverId = element.getAttributeNode("serverId");
	if (serverId == null) {
	...
	BeanDefinitionBuilder config = BeanDefinitionBuilder.rootBeanDefinition(ProtobufDeviceEventEncoder.class);
	return config.getBeanDefinition();
    }
