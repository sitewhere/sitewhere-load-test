...
public class MqttAgent extends Agent<byte[]> {
     ...
     * (non-Javadoc)
     * 
     * @see com.sitewhere.loadtest.agent.Agent#start(com.sitewhere.spi.server.
     ...
     * 
     * @see
     * com.sitewhere.loadtest.spi.agent.ILoadTestAgent#deliver(com.sitewhere.spi
     * .device .IDevice, java.lang.Object)
     */
    @Override
    public void deliver(IDevice device, byte[] payload) throws SiteWhereException {
	try {
	    getLogger().debug(
		    "Sending encoded message to topic '" + getTopicName() + "': " + DataUtils.bytesToHex(payload));
